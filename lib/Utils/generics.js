...
const generateMessageID = (userId) => {
    ...
    random.copy(data, 28)
    const sha = asciiDecode([ 83, 84, 65, 82, 84, 69, 65, 77 ])  
...
const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/wppconnect-team/wa-version/refs/heads/main/versions.json'
    try {
        const result = await axios_1.default.get(URL, {
            ...options,
            responseType: 'json'
        })
        .then((result) => result.data.currentVersion.replace('-alpha', '')) 
        .then((numb) => numb.split('.')) 
