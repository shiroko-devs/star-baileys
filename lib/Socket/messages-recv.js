...
const makeMessagesRecvSocket = (config) => {
    ...
    const { ev, authState, ws, processingMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, groupMetadata, getUSyncDevices, createParticipantNodes, sendPeerDataOperationMessage } = suki
                ...
                processingMutex.mutex(async () => {
                    ...
                    if (msg.messageStubType === WAProto_1.proto.WebMessageInfo.StubType.CIPHERTEXT) {
                        ...
                        const message = msg.message[type_2]
                         ...
                         if (node?.attrs?.addressing_mode === 'lid') {
                            ...
                            if (message?.contextInfo?.participant) {
                            	found = metadata.participants.find(p => p.lid === msg.message[type_2].contextInfo.participant)
                        ...
                        if (!WABinary_1.isJidGroup(node.attrs.from) && WABinary_1.isLidUser(WABinary_1.jidNormalizedUser(node.attrs.from))) {
                            ...
                            if (message?.contextInfo?.participant) {
                            	const isMe = msg.message[type_2].contextInfo.participant === WABinary_1.jidNormalizedUser(authState.creds.me.lid) 
                                msg.message[type_2].contextInfo.participant = isMe ? WABinary_1.jidNormalizedUser(authState.creds.me.id) : senderPn
                            }
                            if (message?.contextInfo?.mentionedJid?.length > 0) {
